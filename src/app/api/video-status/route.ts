...
export async function POST(req: NextRequest) {
  try {
    ...
    if (currentStatus === 'COMPLETED') {
      ...
      // VFX Studio layer job: hand the artifact back to its project document — matte
      // (colour + alpha clips) or O3 edit (the shot) — which composites/grades and moves
      // the shot to review. The generic upload above is kept as the job's own record.
      if (genRow?.metadata?.mode === 'vfx-layer') {
        try {
          const m = genRow.metadata;
          const project = await loadVfxProject(m.vfx_user_email, m.vfx_project_id, supabase);
          if (project) {
            await onLayerJobDone(project, m.vfx_shot_id, m.vfx_layer_id, { video: publicUrl, color: tempUrl, alpha: matteAlphaUrl }, supabase);
            await persistVfx(project, supabase);
          }
        } catch (vfxErr: any) {
          console.error('[VFX layer] post-processing failed:', vfxErr?.message || vfxErr);
        }
      }
      ...
      if (genRow?.metadata?.mode === 'vfx-layer') {
        try {
          ...
          const project = await loadVfxProject(m.vfx_user_email, m.vfx_project_id, supabase);
          if (project) {
            onLayerJobFailed(project, m.vfx_shot_id, m.vfx_layer_id, statusData.error || 'โมเดลรายงานล้มเหลว');
            await persistVfx(project, supabase);
          }
        } catch (vfxErr: any) {
          console.error('[VFX layer] failure bookkeeping failed:', vfxErr?.message || vfxErr);
        }
      }
