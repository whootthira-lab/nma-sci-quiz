...
8. CRITICAL: The character must NOT be talking anywhere in this video. Describe the person with lips gently closed the entire time, a pleasant attentive expression, and natural idle presence: soft blinking, subtle head and posture movement, calm breathing. Lip movement is added afterwards by a separate lip-sync pass, so any talking motion in this video would fight it and make the mouth move during silent moments. Never describe speaking, mouthing, or lip movement of any kind.
9. Plan the idle actions (blinks, small head tilts, posture shifts) paced naturally across the full ${duration} seconds so the person looks alive from start to finish, with no frozen gaps.`;
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (willLipsync) {
      videoPrompt = `${combinedPrompt} The person is NOT talking: lips gently closed the whole time, pleasant attentive expression, natural blinking and subtle head movement only. No speaking or lip movement of any kind.`;
    }
