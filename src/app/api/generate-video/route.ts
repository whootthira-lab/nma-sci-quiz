...
9. The generated visual prompt MUST plan and describe talking actions, facial expression transitions, and posture adjustments that are paced naturally to fit exactly the video duration of ${duration} seconds. Coordinate the speech pace and actions so they are distributed evenly across the ${duration} seconds, leaving no blank/empty gaps at the end or mid-clip, ensuring the speech behavior looks natural and realistic from start to finish.
10. Explicitly adjust the sound details based on camera distance: sounds from foreground characters or close-up actions must be sharp, detailed, and high-definition, whereas sounds from background objects or far-away ambient elements must be low-volume, muffled, and distant.`;
  }
