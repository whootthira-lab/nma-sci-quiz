...
async function enhancePromptWithGPT(
  ...
  let systemMessage = `You are an expert AI Prompt Engineer specialized in video generation models (Wan 2.5, Kling 2.5/2.6, and Grok).
...
2. Expand it to describe cinematic details: camera angle (framing, camera distance from subject, depth of field, e.g., close-up, medium shot, extreme close-up, background vs. foreground focus), lighting (cinematic, soft, shadows), details of the subject, and background atmosphere matching the situation.
...
6. The generated visual prompt MUST plan and describe visual actions, object movements, and camera panning that are paced naturally to fit exactly the video duration of ${duration} seconds. Distribute the actions evenly across the ${duration} seconds timeframe to ensure the video has continuous motion and looks realistic from start to finish, leaving no idle or frozen gaps at the end or mid-clip.`;

  if (!isNoSpeech) {
    systemMessage = `You are an expert AI Prompt Engineer specialized in video generation models (Wan 2.5, Kling 2.5/2.6, and Grok).
...
Guidelines:
1. Translate the situation description to English if it is in Thai.
2. Expand it to describe cinematic details: camera angle (medium close-up, close-up, talking portrait, camera distance from subject, depth of field), lighting (soft studio light, cinematic lighting), details of the person (natural facial features, natural blinking, high detail skin texture), and background atmosphere matching the situation.
...
4. Do NOT include the actual spoken words inside the prompt itself, only describe the visual scene.
...
7. Describe realistic ambient sounds from the scene (e.g., wind blowing, indoor hum, background music) and sound effects generated by the actions of the characters or objects (e.g., footsteps, cloth rustling) matching the situation, at quiet background volume.
8. CRITICAL: The character must NOT be talking anywhere in this video — lips stay closed, zero lip or mouth movement. Lip movement is added afterwards by a separate lip-sync pass, so any talking motion here would fight it and make the mouth move during silent moments. Never describe speaking, mouthing, or lip movement of any kind.
9. Everything EXCEPT the lips must be vividly expressive — the earlier neutral-idle style read as stiff and lifeless on screen. Direct the person like an energetic children's TV presenter: lively animated eyebrows, wide expressive eyes, big warm smiles, enthusiastic head nods and tilts, shoulders and hand gestures acting out the meaning of what is being said, slightly exaggerated theatrical expressions matching the emotion of the script. Pace these gestures across the full ${duration} seconds so the person looks alive from start to finish, with no frozen gaps.`;
  }
...
function getWanVideoParams(targetSeconds: number) {
  // Lock num_frames to exactly 81 to avoid Fal.ai 1.25x billing multiplier.
  const frames = 81;
  let bestFps = 16;
  ...
  return { num_frames: frames, frames_per_second: bestFps };
}
  ...
  try {
    ...
    try {
      const { data: settings } = await supabase
        ...
        .select('key, value');
      if (settings) {
        const provider = settings.find((item: any) => item.key === 'open_source_provider');
    ...
    if (modelType === 'cinema') {
      ...
      ratePerSecond = grokResolution === '480p' ? 4 : 6;         // billed well under list so far
    } else if (modelType === 'fast') {
      if (klingResolution === '1080p') {
        ratePerSecond = klingAudioEnabled ? 15 : 8;              // $0.14 / $0.07 per second
      } else {
        ratePerSecond = 5;                                       // $0.042 per second
      }
    } else if (modelType === 'seedance') {
      ratePerSecond = seedanceResolution === '480p' ? 3 : (seedanceResolution === '1080p' ? 13 : 6);
    } else if (modelType === 'elements') {
      ratePerSecond = 10;                                        // $0.098 per second
    ...
    // The mouth belongs to the lip-sync pass alone: if the base video talks on its own,
    // its lips leak through wherever the audio is silent — most visibly during the 1s
    // lead-in — which reads as "ปากไม่ตรงเสียง". Appended outside the enhancer so the
    // rule also holds when the user opts out of enhancement.
    let videoPrompt = combinedPrompt;
    if (willLipsync) {
      // Lips belong to the lip-sync pass — but ONLY the lips. The earlier wording asked
      // for a subtle idle presence and the result read as stiff: a face that neither
      // moved nor reacted while the mouth talked. Everything except the lips is now
      // pushed the other way, toward a children's-TV-host register.
      videoPrompt = `${combinedPrompt} The person is NOT talking — lips stay closed, absolutely no lip or mouth movement — but everything else is highly expressive and animated, like an energetic children's TV presenter: lively eyebrows, wide expressive eyes, big warm smiles, enthusiastic head nods and tilts, shoulders and hand gestures that act out the mood, slightly exaggerated theatrical expressions. Never speaking, never mouthing words.`;
    } else if (narrationOnly && !isNoSpeech) {
      // Narration plays over the scene, so anyone visible must not appear to speak
      videoPrompt = `${combinedPrompt} No one in the scene is talking to the camera: any visible people keep their lips closed, with natural lively movement and expressions. This is footage to be narrated over.`;
    }
          ...
          : (isMotionControlModel 
              ...
              : (isGrok 
                  ...
                  : (klingResolution === '1080p'
                      ? (videoMode === 'text_to_video' ? 'fal-ai/kling-video/v2.6/pro/text-to-video' : 'fal-ai/kling-video/v2.6/pro/image-to-video')
